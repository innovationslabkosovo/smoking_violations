...
class Location_Model extends ORM
{
	...
	public static function get_locations_date($where = array(), $limit = 0)
	{
		// Database table prefix
		$table_prefix = Kohana::config('database.default.table_prefix');
		
		// SQL query
		$sql = 'SELECT location.id, location_name AS name, country_id, latitude, longitude, max(incident_date) AS inc_date FROM  '.$table_prefix.'location,  '.$table_prefix.'incident WHERE location_visible = 1 and location_id = location.id group by location_name ';
		
		// Check for parameters
		
		
		// Order the records by database ID
		$sql .= 'ORDER BY inc_date DESC ';
